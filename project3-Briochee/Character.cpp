...
//Project 3: ArrayBag class implementation
...
//project 3 methods
//project 3 operators
//returns true if right hand side chracter is equal, false otherwise
//checks name, race, level, and enemy for equality
bool Character::operator == (const Character& character) const{
    bool equalName = (this->getName() == character.getName());
    bool equalRace = (this->getRace() == character.getRace());
    bool equalLevel = (this->getLevel() == character.getLevel());
    bool equalEnemy = (this->isEnemy() == character.isEnemy());
    
    //returns true if name, race, level, and enemy are equal
    if (equalName && equalRace && equalLevel && equalEnemy){
        return true;
    }
    return false;
}

//returns true if right hand side chracter is not equal, false otherwise
//checks name, race, lecel, and enemy for equality
bool Character::operator != (const Character& character) const{
    //code copied from == operator overload
    bool equalName = (this->getName() == character.getName());
    bool equalRace = (this->getRace() == character.getRace());
    bool equalLevel = (this->getLevel() == character.getLevel());
    bool equalEnemy = (this->isEnemy() == character.isEnemy());
